...
if [ "$1" -eq 0 ]; then
	while read line
		do
			...
			$YTDL -f best  -x --postprocessor-args "-ss 00:00:00.00 -t 00:02:00.00" "http://www.youtube.com/watch?v=$line" -o ./videos/"$name%(title)s-%(id)s.%(ext)s"
	...
	while read line
		do
			if [ "$limit" -gt 0 ]; then
				limit=$(($limit-1))
				$YTDL -f best -x --postprocessor-args "-ss 00:00:00.00 -t 00:02:00.00"  "http://www.youtube.com/watch?v=$line" -o ./videos/"$name%(title)s-%(id)s.%(ext)s" 2>&1 | tee log.txt
